...
CREATE OR REFRESH STREAMING TABLE sdp_1_bronze.orders_bronze_demo6
...
CREATE OR REFRESH STREAMING TABLE sdp_2_silver.orders_silver_demo6
  (
...
SELECT 
  ...
  customer_id,
  notifications
FROM STREAM sdp_1_bronze.orders_bronze_demo6; 


-- Create the materialized view aggregation from the orders_silver table with the summarization
CREATE OR REFRESH MATERIALIZED VIEW sdp_3_gold.orders_by_date_gold_demo6 
...
FROM sdp_2_silver.orders_silver_demo6  
